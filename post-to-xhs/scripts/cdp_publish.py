...
XHS_IMAGE_TEXT_TITLE_LIMIT = 20


def normalize_image_text_title(title: str, max_chars: int = XHS_IMAGE_TEXT_TITLE_LIMIT) -> str:
    normalized = " ".join(title.split()).strip()
    if len(normalized) <= max_chars:
        return normalized

    candidates = []
    for pattern in (r"^HN热帖[:：]\s*", r"^HN[:：]\s*", r"^Hacker News[:：]\s*"):
        compact = re.sub(pattern, "", normalized, flags=re.IGNORECASE)
    ...
    for pattern in (
        ...
        compact = re.sub(pattern, "", normalized)
        if compact != normalized:
            candidates.append(compact)

    compact_whitespace = normalized.replace(" · ", " ").replace(" | ", " ").strip()
    if compact_whitespace != normalized:
        candidates.append(compact_whitespace)

    compact_cjk = re.sub(r"(?<=[\u4e00-\u9fff])\s+(?=[A-Za-z0-9])|(?<=[A-Za-z0-9])\s+(?=[\u4e00-\u9fff])", "", normalized)
    if compact_cjk != normalized:
        candidates.append(compact_cjk)
    ...
    base = candidates[-1] if candidates else normalized
...
class XiaohongshuPublisher:
    ...
    def _fill_title(self, title: str):
        ...
        normalized_title = normalize_image_text_title(title)
        if normalized_title != title:
            print(
                f"[cdp_publish] Title too long for image-text mode ({len(title)} chars). "
                f"Using normalized title: {normalized_title}"
            )
    ...
    def _click_publish(self):
        ...
        if clicked:
            ...
            while time.time() < deadline:
                ...
                current_url = (state or {}).get("url", "")
                body = (state or {}).get("body", "")
                if "/publish/success" in current_url or "发布成功" in body:
                    print("[cdp_publish] Publish confirmed by success page.")
                    return
                time.sleep(0.5)

            raise CDPError(
                "Publish button was clicked, but success page was not detected. "
                "Please verify whether the note was actually published."
            )
        else:
            raise CDPError(
