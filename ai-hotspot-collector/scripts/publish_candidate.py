#!/usr/bin/env python3
"""
Publish a reviewed aggregated candidate draft via article-multi-publisher.
"""
...
def resolve_candidate_by_index(root: Path, index: int) -> Path:
    ...
    for item in manifest:
        if int(item.get("global_index", -1)) != index:
            continue
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Publish a reviewed aggregated draft via article-multi-publisher.")
    parser.add_argument("path", help="Aggregated content root, candidate directory, or reviewed draft file")
    parser.add_argument("--index", type=int, help="Global candidate number from manifest.json when the input is an aggregated root directory")
