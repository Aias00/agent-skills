...
摘要要求（自然口语化）：
1. 2-3 句中文摘要，80-150 字
2. 语言风格：自然、口语化、像人写的，避免机械感
3. 禁止使用的机械短语：
   - "旨在"（替换为："用来"、"为了"）
   - "此举表明"（替换为："这"、"这意味着"）
   - "猛然发展"（替换为："快速发展"）
...
4. 内容要求：
   ...
   - 说明重要性或影响（为什么重要）
   - 提供具体细节（时间、地点、人物、数据等）
5. 不要空洞的分析模板（如"行业动态类新闻的价值在于..."）
6. 如果与 AI 无关，`summary` 直接写 `与AI无关`

返回格式要求：
- 必须是 JSON，对象包含 `score` 和 `summary`
- `score` 是 0 到 10 的数字
- 不要输出 JSON 以外的任何文字

示例（好的摘要）：
- "马斯克的 xAI 正在重新开发 AI 编码工具，从 Cursor 挖来两名高管提升竞争力。这表明 xAI 在竞争激烈的 AI 编码助手市场寻求突破。"
- "微软将 Copilot AI 助手带到 Xbox 主机，让玩家可以用语音解决游戏难题。这意味着游戏助手功能开始进入大众游戏场景。"

示例（不好的摘要）：
- "马斯克的 AI 公司再次开始开发编码工具，旨在提升其竞争力。此举表明该公司在 AI 编码领域进行战略调整。"（使用了"旨在"、"此举表明"等机械短语）
- "这类公司动作类新闻的价值在于行业动态的变化。"（空洞的分析模板）
"""
