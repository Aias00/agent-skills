...
from config import PUBLISH_URL, DEBUG_SCREENSHOT_DIR
...
def sanitize_html_fragment(html_text):
    """Keep only a safe body fragment for Toutiao editor paste."""
    fragment = html_text

    body_match = re.search(
        r"<body[^>]*>(.*)</body>", fragment, flags=re.IGNORECASE | re.DOTALL
    )
    if body_match:
        fragment = body_match.group(1)

    fragment = re.sub(
        r"<script\b[^>]*>.*?</script>", "", fragment, flags=re.IGNORECASE | re.DOTALL
    )
    fragment = re.sub(
        r"<style\b[^>]*>.*?</style>", "", fragment, flags=re.IGNORECASE | re.DOTALL
    )
    fragment = re.sub(r"<img\b[^>]*>", "", fragment, flags=re.IGNORECASE)
    fragment = re.sub(r"<(meta|link)\b[^>]*>", "", fragment, flags=re.IGNORECASE)
    fragment = re.sub(
        r"</?(html|head|body)\b[^>]*>", "", fragment, flags=re.IGNORECASE
    )

    return fragment.strip()


def infer_cover_from_content_path(content_path):
    """Try to resolve a local cover image near the article file."""
    if not content_path:
        return None
    ...
    search_dirs = [content_path.parent, content_path.parent / "images"]
    preferred_names = [
        "cover.png",
        "cover.jpg",
        "cover.jpeg",
    ...
    for directory in search_dirs:
        if directory.exists():
            for pattern in ("*.png", "*.jpg", "*.jpeg", "*.webp"):
                image_candidates.extend(sorted(directory.glob(pattern)))
...
def publish(
    ...
    if content_html:
        if content_is_html:
            print("🔄 Using provided HTML content directly...")
            final_html = sanitize_html_fragment(content_html)
        elif raw:
            final_html = raw_text_to_html(content_html)
        else:
            ...
            try:
                ...
                print(f"⚠️ Conversion failed, using safe raw text: {e}")
                final_html = raw_text_to_html(content_html)
    ...
    with sync_playwright() as p:
        ...
        def take_screenshot(name):
            ...
            try:
                ...
                filename = DEBUG_SCREENSHOT_DIR / f"debug_{name}_{ts}.png"
        ...
        try:
            ...
            if content_html:
                ...
                try:
                    ...
                    if editor.count() > 0:
                        ...
                        # Save may still fail here if required fields like cover are missing.
                        print("  Checking save status...")
                        page.wait_for_timeout(2000)
                        if page.get_by_text("草稿已保存").is_visible():
                            print("✅ Draft saved successfully.")
                        elif page.get_by_text("保存失败").is_visible():
                            print(
                                "⚠️ Draft save failed before required fields were complete. Continuing to cover upload..."
                            )
                            take_screenshot("save_failed")
                        else:
                            print("⚠️ Draft save status not visible yet. Continuing...")
            ...
            if cover_image_path:
                ...
                try:
                    ...
                    if not os.path.exists(cover_image_path):
                        ...
                        # Ensure single-cover mode is active.
                        try:
                            single_cover = (
                                page.locator("label.byte-radio").filter(has_text="单图").last
                            )
                            if single_cover.is_visible():
                                single_cover.click()
                                time.sleep(1)
                        except Exception as radio_error:
                            print(f"  ⚠️ Could not force single-cover mode: {radio_error}")
                        ...
                        if page.get_by_text("草稿已保存").is_visible():
                            print("✅ Draft saved after cover upload.")
                        elif page.get_by_text("保存失败").is_visible():
                            print("⚠️ Save warning still visible after cover upload.")
            ...
            if not dry_run:
                ...
                try:
                    ...
                    print(f"  Current URL after publish attempt: {page.url}")
                    try:
                        body_text = page.locator("body").inner_text(timeout=5000)
                        print("  Page text snippet after publish attempt:")
                        print(body_text[:1000])
                    except Exception as body_error:
                        print(f"  ⚠️ Could not capture body text: {body_error}")
            ...
            if not headless and wait_seconds > 0:
                print(f"browser open for inspection. Closing in {wait_seconds}s...")
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Select 'No Cover' option",
    )
    parser.add_argument(
        "--with-cover",
        dest="no_cover",
        action="store_false",
        help="Force with-cover mode (overrides EXTEND.md default_no_cover)",
    )
    parser.add_argument(
        "--raw",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Paste content as raw text (no markdown conversion)",
    )
    parser.add_argument(
        "--debug-screenshots",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Save debug screenshots to skill data directory",
    )
    parser.add_argument(
        "--wait-seconds",
        type=int,
    ...
    headless = (
        ...
        else to_bool(extend.get("default_headless"), False)
    )
    no_cover = (
        args.no_cover
        if args.no_cover is not None
        else to_bool(extend.get("default_no_cover"), False)
    )
    ...
    wait_seconds = (
        ...
        if args.wait_seconds is not None
            ...
            extend.get("default_wait_seconds"),
            0 if headless else 60,
        )
    )
    ...
    if content and os.path.exists(content):
        ...
        content_is_html = content_path.suffix.lower() in {".html", ".htm"}
        if not cover_image_path and not no_cover:
            cover_image_path = infer_cover_from_content_path(content_path)
            if cover_image_path:
                print(f"🖼️ Auto-resolved cover image: {cover_image_path}")
