...
class PublishNewDeckTests(unittest.TestCase):
    def test_publish_new_deck_uses_slides_create_with_batch_when_slide_count_is_small(self):
        module = load_module()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with mock.patch.object(module, "run_lark_cli", return_value=read_fixture("presentation-create.json")):
                result = module.publish_slides(outline, slides, pathlib.Path(temp_dir), None)
            ...
            self.assertTrue((pathlib.Path(temp_dir) / "publish-result.json").exists())
